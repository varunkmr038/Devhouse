...
const setNotificationToUnread = async (userId) => {
  try {
    ...
    if (!user.unreadNotification) {
      user.unreadNotification = true; // set user unread notification to true
      await user.save();
    }
...
const newLikeNotification = async (userId, postId, userToNotifyId) => {
  try {
    //  Finding userNotifcation document  from notification model
    ...
    //  Add notification in notifications of the user
...
const removeLikeNotification = async (userId, postId, userToNotifyId) => {
  try {
    ...
    //  find the first notification doc with given values
    ...
    const indexOf = user.notifications
      ...
      .indexOf(notificationToRemove._id.toString()); // finding the index of notification to remove

    await user.notifications.splice(indexOf, 1);
    await user.save();

    //  Can also remove like this
    // await NotificationModel.findOneAndUpdate(
    //   { user: userToNotifyId },
    //   {
    //     $pull: {    // removes docs with array with condition
    //       notifications: {
    //         type: "newLike",
    //         user: userId,
    //         post: postId
  ...
  try {
    ...
    const notificationToRemove = await user.notifications.find(
      (notification) =>
        ...
        notification.user.toString() === userId &&
        notification.post.toString() === postId &&
        notification.commentId === commentId // user can have multiple comments on post
    );
